...
const ArtworksPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<
    "latest" | "oldest" | "price_high" | "price_low"
  >("latest");
        ...
        const mapped = clientsArray.map((c: any, index: number) => {
          ...
          if (c.data) {
            Object.keys(c.data).forEach((key) => {
              if (
                !["name", "phone", "email", "address", "note"].includes(key)
              ) {
  ...
  }, [searchTerm, selectedArtist, sortBy, currentPage]);

  // 서버에서 이미 필터링과 정렬이 적용된 결과를 사용
  ...
  const handleArtworkAction = (
    actionType: "edit" | "delete" | "certificate",
    artwork: any,
  ) => {
  ...
  const artistList = useMemo(() => {
    // 한국어와 영어 작가명 모두 수집
    const allArtists = artworks
      .map((a) => a.artist_ko || a.artist_en)
      .filter(Boolean);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="작품 관리"
        description="전시 및 판매 작품을 관리하고 보증서를 발급하세요."
      >
        <Dialog
          open={isAddArtworkDialogOpen}
          onOpenChange={setIsAddArtworkDialogOpen}
        >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100">
        <ArtworksTable
          ...
          handleArtworkAction={handleArtworkAction}
        />
        {/* 페이지네이션 */}
