...
const ClientsPage: React.FC = () => {
  ...
  const displayedClients = filteredRows.slice(
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (
        ...
        const serverData = Array.isArray(serverResponse.results)
  ...
  useEffect(() => {
    ...
    fetchClientsServerSide({
      ...
      .then((data) => {
        ...
        console.log(
          "⚠️ 컬럼 생성을 TanStackDynamicTable에 위임 - 중복 생성 방지",
        );
      ...
      <TanStackDynamicTable
        ...
        onAddRow={async (row) => {
          ...
          if (row.tags || row["고객분류"]) {
            ...
            console.log(
              '🏷️ 태그 없음 - 백엔드에서 자동으로 "일반고객" 태그 할당됨',
            );
          }
        ...
        onDeleteRow={async (id) => {
          console.log(
            "🚨 onDeleteRow 호출됨 - 이 콜백은 사용되지 않아야 합니다!",
          );
        ...
        onSimpleExcelUpload={async (file) => {
          ...
          try {
            ...
            const { authenticatedFetch } = await import("@/utils/api");
            const response = await authenticatedFetch(
              `${API_BASE_URL}/excel/upload-with-mapping/`,
              {
                method: "POST",
                body: formData,
              },
