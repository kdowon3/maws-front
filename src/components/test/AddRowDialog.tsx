...
const AddRowDialog: React.FC<AddRowDialogProps> = ({ open, onOpenChange, columns, onSave }) => {
    ...
    const initValues = () => {
        ...
        columns.forEach((col) => {
            ...
            else if (col.meta.type === 'tags') obj[col.id] = null; // 태그는 null로 초기화 (단일 선택)
            else obj[col.id] = '';
        });
        return obj;
    };
    ...
    const handleSave = () => {
        ...
        // 태그 컬럼을 모두 tags 배열로 합치고, data에는 넣지 않음
        const processedValues = { ...values };
        const tagsColumns = columns.filter((col) => col.meta.type === 'tags');
        let tags = [];
        tagsColumns.forEach((col) => {
            const selectedTag = values[col.id];
            if (selectedTag) tags.push(selectedTag);
            // data에서 태그 관련 필드는 삭제
            delete processedValues[col.id];
        });
        processedValues.tags = tags;
