...
export default function TanStackDynamicTable({
  ...
  const refreshColumnsAndData = async (notifyParent = false) => {
    // 컬럼 수정 중이면 새로고침 건너뛰기
    if (isEditingColumn) {
      return;
    }
    try {
      ...
      // 필수 컬럼 정의 - 기본 order 값은 참조용으로만 사용
      const requiredColumns = [
        { id: "고객명", header: "고객명", meta: { type: "text" }, order: 0 },
        { id: "연락처", header: "연락처", meta: { type: "text" }, order: 1 },
        {
          id: "고객분류",
          header: "고객분류",
          meta: { type: "tags" },
      ...
      // 서버 컬럼과 현재 props 컬럼을 매핑 (accessor와 header 모두 고려)
      const serverColumnMap = new Map();
      cols.forEach((c: any) => {
        const accessor = c.accessor || c.id;
        const header = c.header;
        serverColumnMap.set(accessor, c);
        serverColumnMap.set(header, c); // header도 키로 추가
      });
      const propsColumnMap = new Map(initialColumns.map((c) => [c.id, c]));

      // 모든 컬럼을 order 기준으로 매핑
      const allColumns: any[] = [];

      // 1. 서버의 모든 컬럼을 가져오기 (필수 컬럼은 order 강제 고정)
      cols.forEach((c: any) => {
        // 🔧 일관된 데이터 매핑을 위해 header를 id로 사용 (기존 EditClientDialog와 호환)
        const columnId = c.header; // header를 id로 사용하여 기존 코드와 호환성 유지
        const propsColumn = propsColumnMap.get(columnId);
        const requiredCol = requiredColumns.find((req) => req.id === columnId);

        console.log(
          `🔍 서버 컬럼 매핑: ${c.header} -> accessor: ${c.accessor} -> columnId: ${columnId}`,
        );

        // 🔧 accessor와 header가 다른 경우 warning 출력
        if (c.accessor && c.accessor !== c.header) {
          console.warn(
            `⚠️ accessor와 header 불일치: ${c.header} vs ${c.accessor}`,
          );
        }

        allColumns.push({
          id: columnId, // 🔧 header를 id로 사용하여 EditClientDialog와 호환성 유지
          header: c.header || propsColumn?.header || columnId || "컬럼",
          meta: { type: c.type || propsColumn?.meta?.type || "text" },
          dbId: c.id,
          accessor: c.accessor, // 🔧 서버 accessor 정보 보존 (참조용)
          order:
            c.order !== undefined
              ? c.order
              : requiredCol
      ...
      // 2. 필수 컬럼이 서버에 없으면 서버에 생성하여 동기화
      // 필수 컬럼의 한국어/영어 별칭 매핑
      const requiredColumnAliases = {
        고객명: ["고객명", "customer_name", "name"],
        연락처: ["연락처", "phone"],
        고객분류: ["고객분류", "tags", "고객 분류"],
      };

      const requiredColumnIds = new Set(requiredColumns.map((c) => c.id));
      for (const reqCol of requiredColumns) {
        // 필수 컬럼이나 그 별칭이 서버에 있는지 확인
        const aliases = requiredColumnAliases[reqCol.id] || [reqCol.id];
        const existsOnServer = aliases.some((alias) =>
          serverColumnMap.has(alias),
        );

        if (!existsOnServer) {
          console.log(`📝 필수 컬럼을 서버에 생성: ${reqCol.id}`);
          try {
            const propsCol = propsColumnMap.get(reqCol.id);
            const result = await createClientColumn({
              header: propsCol?.header || reqCol.header,
              accessor: propsCol?.header || reqCol.header, // 🔧 header와 동일하게 설정하여 데이터 매핑 일관성 유지
              type: propsCol?.meta?.type || reqCol.meta.type || "text",
              order: reqCol.order,
            });
            console.log(
              `✅ 필수 컬럼 생성: ${reqCol.id} -> accessor: ${propsCol?.header || reqCol.header}`,
            );

            console.log(`✅ 필수 컬럼 서버 생성 완료: ${reqCol.id}`, result);

            allColumns.push({
              id: reqCol.id,
              header: propsCol?.header || reqCol.header,
              meta: { type: propsCol?.meta?.type || reqCol.meta.type },
              dbId: result.id,
              order: reqCol.order,
            });
          } catch (err) {
            console.error(`❌ 필수 컬럼 서버 생성 실패: ${reqCol.id}`, err);
            // 실패 시 dbId 없이 추가
            const propsCol = propsColumnMap.get(reqCol.id);
            allColumns.push({
              id: reqCol.id,
      ...
      // 3. props에만 있고 서버에 없는 컬럼들을 서버에 생성하여 동기화 (이미 처리한 컬럼 제외)
      const processedColumnIds = new Set([
        ...Array.from(serverColumnMap.keys()), // 기존 서버 컬럼
        ...requiredColumns.map((c) => c.id), // 방금 처리한 필수 컬럼들
      ]);

      const propsOnlyColumns = initialColumns.filter(
        (propsCol) =>
          !processedColumnIds.has(propsCol.id) &&
          !deletedColumns.has(propsCol.id) &&
          !["tag_ids", "data", "created_at", "updated_at", "id"].includes(
            propsCol.id,
          ),
      );

      for (const propsCol of propsOnlyColumns) {
        try {
          const initialIndex = initialColumns.findIndex(
            (c) => c.id === propsCol.id,
          );
          const newOrder = Math.max(3, initialIndex + 3); // 필수 컬럼 이후 배치

          const result = await createClientColumn({
            header: propsCol.header,
            accessor: propsCol.header, // 🔧 header와 동일하게 설정하여 데이터 매핑 일관성 유지
            type: propsCol.meta.type || "text",
            order: newOrder,
          });
          console.log(
            `✅ props 컬럼 생성: ${propsCol.id} -> accessor: ${propsCol.header}`,
          );
      ...
      // 4. 중복 컬럼 제거 (더 엄격한 중복 제거)
      const uniqueColumnsMap = new Map();
      const duplicateLogs = [];

      // 먼저 header 기준으로 중복 제거
      for (const col of allColumns) {
        const headerKey = col.header.toLowerCase().trim();
        const existing = uniqueColumnsMap.get(headerKey);
        ...
        if (!existing) {
          ...
          // 1. dbId가 있는 컬럼 우선 (서버에 저장된 컬럼)
          if (col.dbId && !existing.dbId) {
            shouldReplace = true;
          }
          // 2. order가 낮은 컬럼 우선
          else if (col.order < existing.order) {
            shouldReplace = true;
          }
          // 3. order가 같으면 id로 비교
          else if (
            col.order === existing.order &&
            col.id.localeCompare(existing.id) < 0
          ) {
            shouldReplace = true;
          }
  ...
  const removeRow = async (rowId: string) => {
    ...
    console.log(
      "⚠️ removeRow는 더 이상 사용되지 않습니다. onAction 콜백을 통해 처리하세요.",
    );
  ...
  const setCell = async (rowId: string, colId: string, value: any) => {
    setData(
      data.map((row) => (row.id === rowId ? { ...row, [colId]: value } : row)),
    );
  ...
  const saveColEdit = async (colId: string) => {
    ...
    const newHeader = editColNameRef.current
      ? editColNameRef.current.value
      : "";
    ...
    const updatedCols = columns.map((col) =>
      col.id === colId
        ? { ...col, header: newHeader, meta: { type: editColType } }
        : col,
    );
    ...
    if (target?.dbId) {
      try {
        ...
        if (newHeader !== oldAccessor) {
          ...
          const refreshedColumns = updatedCols.map((col) =>
            col.id === colId
              ? { ...col, accessor: newHeader, id: newHeader }
              : col,
          );
        ...
        toast.error(
          `컬럼 수정에 실패했습니다: ${err?.message || "알 수 없는 오류"}`,
        );
      ...
      alert(
        "⚠️ 이 컬럼은 서버에 동기화되지 않았습니다. 페이지를 새로고침 후 다시 시도해 주세요.",
      );
  ...
  function guessType(values: any[]) {
    ...
    if (values.every((v) => v === "" || v == null || !isNaN(Number(v))))
      return "number";
    if (
      values.every(
        (v) => v === "" || v == null || /^\d{4}[-./]\d{2}[-./]\d{2}$/.test(v),
      )
    )
      return "date";
    return "text";
      ...
      ...columns.map((col) => ({
        ...
        header: () => (
          ...
          >
            ...
            <Edit2
              size={14}
              className="inline ml-1 text-gray-400 group-hover:text-blue-500"
            />
            {popoverCol === col.id && (
              <div
                ref={popoverRef}
        ...
        cell: ({ row }) => {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `🔍 셀 렌더링: 컬럼=${col.id} (헤더=${col.header}), 값=${value}, 타입=${typeof value}`,
            );
            if (value === undefined || value === null) {
              console.warn(
                `⚠️ 빈 값 감지: 컬럼=${col.id}, row.original=`,
                row.original,
              );
            }
          }
          ...
          if (col.meta.type === "tags") {
            return (
              <div className="min-w-[280px] max-w-[400px] py-1">
                <ClientTags
        ...
        size: col.meta.type === "tags" ? 120 : 150, // 텍스트가 한 줄에 들어가도록
        minSize: col.meta.type === "tags" ? 100 : 120,
        maxSize: col.meta.type === "tags" ? 180 : 300,
      })),
      // 액션 버튼 컬럼 정의
      {
        id: "add",
  ...
  const handleSaveRow = async (rowData: any) => {
    if (onAddRow) {
      ...
      // fallback: 로컬 상태 업데이트
      const newRow: TableRow = { id: uuid(), ...rowData };
      setData([...data, newRow]);
    }
  ...
  React.useEffect(() => {
    const initializeColumns = async () => {
      try {
        ...
        await refreshColumnsAndData(false); // 초기화 시에는 상위 컴포넌트에 알리지 않음
  ...
  React.useEffect(() => {
    console.log(
      "TanStackDynamicTable - initialData 변경:",
      initialData?.slice(0, 3) || initialData,
    );
    setData(initialData);
  ...
  React.useEffect(() => {
        ...
        headers.forEach((header: string, idx: number) => {
          ...
          const similarColumn = columns.find((col) => {
            ...
            return (
              ...
              (colHeader === "고객명" &&
                ["이름", "성함", "성명", "고객명"].includes(uploadHeader)) ||
              (colHeader === "연락처" &&
                ["전화", "휴대폰", "핸드폰", "연락처", "전화번호"].includes(
                  uploadHeader,
                )) ||
              (colHeader === "고객분류" &&
                ["분류", "구분", "카테고리", "타입", "종류"].includes(
                  uploadHeader,
                ))
            );
          });
          ...
          initialMappings[cleanHeader] = similarColumn
            ? similarColumn.header
            : `new_${idx}`;
        });
    ...
    return () =>
      window.removeEventListener(
        "triggerColumnMapping",
        handleColumnMappingEvent,
      );
  }, [columns]);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex gap-2 mb-4">
          ...
          <Button
            ...
            onClick={() =>
              document.getElementById("excel-upload-simple-input")?.click()
            }
          >
            📊 엑셀 업로드
          </Button>
