...
export default function TanStackDynamicTable({
    ...
    const addColumn = async () => {
        ...
        try {
            console.log('🔄 컬럼 추가 시도:', { header: '새 컬럼', accessor, type: 'text', order: newOrder });
            console.log('🌐 API 호출: createClientColumn');
            const res = await createClientColumn({ header: '새 컬럼', accessor, type: 'text', order: newOrder });
            ...
            console.error('❌ 에러 상세:', { message: err?.message, stack: err?.stack });
    ...
    const refreshColumnsAndData = async (notifyParent = false) => {
        ...
        try {
            ...
            const requiredColumns = [
                ...
                { id: '고객분류', header: '고객분류', meta: { type: 'tags' }, order: 2 },
            ];
            ...
            cols.forEach((c: any) => {
                ...
                console.log(`🔍 서버 컬럼 매핑: ${c.header} -> accessor: ${c.accessor} -> columnId: ${columnId}`);
            ...
            for (const reqCol of requiredColumns) {
                ...
                const existsOnServer = aliases.some((alias) => serverColumnMap.has(alias));
            ...
            const propsOnlyColumns = initialColumns.filter(
                (propsCol) =>
                    ...
                    !['tag_ids', 'data', 'created_at', 'updated_at', 'id'].includes(propsCol.id)
            );

            for (const propsCol of propsOnlyColumns) {
                try {
                    const initialIndex = initialColumns.findIndex((c) => c.id === propsCol.id);
                    ...
                    console.log(`✅ props 컬럼 생성: ${propsCol.id} -> accessor: ${propsCol.header}`);
            ...
            for (const col of allColumns) {
                ...
                if (!existing) {
                    ...
                    // 3. order가 같으면 id로 비교
                    else if (col.order === existing.order && col.id.localeCompare(existing.id) < 0) {
                        shouldReplace = true;
                    }
    ...
    const openPopover = (col: TableColumn) => {
        ...
        setTimeout(() => {
            ...
            console.log('📝 팝오버 상태 설정 완료:', { colId: col.id, type: col.meta.type });
        }, 0);
    };
    ...
    const saveColEdit = async (colId: string) => {
        ...
        if (target?.dbId) {
            try {
                console.log('🔄 컬럼 수정 시도:', { colId, dbId: target.dbId, header: newHeader, type: editColType });
                ...
                cell: ({ row }) => {
                    ...
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        if (value === undefined || value === null) {
                            console.warn(`⚠️ 빈 값 감지: 컬럼=${col.id}, row.original=`, row.original);
                        }
                    }
