...
const CaptionTestPage: React.FC = () => {
    const [selectedDataset, setSelectedDataset] = useState<keyof typeof testScenarios>('basic');
    const [selectedTemplate, setSelectedTemplate] = useState<keyof typeof availableTemplates>('Template 1');
    ...
    const finalData = selectedDataset === 'basic' 
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <div className="mt-2 flex items-center gap-2 text-sm">
                        <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full">✨ 자동 공간 최적화</span>
                        <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full">🚫 캡션 절단 방지</span>
                        <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-full">📊 실시간 통계</span>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedDataset(e.target.value as keyof typeof testScenarios)}
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setSelectedTemplate(e.target.value as keyof typeof availableTemplates)}
                                className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            >
                    ...
                    <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                        <div className="flex items-center justify-between">
                            <div className="text-sm text-blue-800">
                                <strong>현재 설정:</strong> {datasetInfo[selectedDataset]} • {selectedTemplate} • 총 {finalData.length}개 캡션
                            </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border p-4 mb-6">
                    ...
                    {/* 데이터셋 빠른 선택 */}
                    <div className="mb-4">
                        <h4 className="text-sm font-medium text-gray-600 mb-2">📊 데이터셋</h4>
                    ...
                    {/* 템플릿 빠른 선택 */}
                    <div>
                        <h4 className="text-sm font-medium text-gray-600 mb-2">🎨 템플릿</h4>
                        <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
                            {Object.keys(availableTemplates).map((templateName) => (
                                <button
                                    key={templateName}
                                    onClick={() => setSelectedTemplate(templateName as keyof typeof availableTemplates)}
                ...
                <details className="mt-6 bg-gray-100 rounded-lg p-4">
                    ...
                    <div className="mt-4 space-y-2 text-sm font-mono">
                        <div><strong>데이터셋:</strong> {selectedDataset}</div>
                        <div><strong>템플릿:</strong> {selectedTemplate}</div>
                        <div><strong>총 캡션 수:</strong> {finalData.length}</div>
                        <div><strong>첫 번째 캡션:</strong></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-yellow-700">
                        <div>
                            ...
                            <ul className="space-y-1">
                                <li>• <strong>단일 캡션</strong>: 템플릿 기본 디자인 확인</li>
                                <li>• <strong>소량 테스트</strong>: 2단 컬럼 배치 확인</li>
                                <li>• <strong>표준 테스트</strong>: 페이지 넘김 로직 확인</li>
                                <li>• <strong>크기 다양성</strong>: 다양한 캡션 크기 대응</li>
                                <li>• <strong>대량 테스트</strong>: 성능 및 다중 페이지</li>
                            </ul>
                        </div>
                        <div>
                            <h4 className="font-medium mb-2">🎨 템플릿별 테스트</h4>
                            <ul className="space-y-1">
                                <li>• <strong>Template 1-8</strong>: 각각 다른 레이아웃 스타일</li>
                                <li>• <strong>빠른 전환</strong>: 상단 그리드에서 클릭으로 변경</li>
                                <li>• <strong>실시간 반영</strong>: 템플릿 변경 즉시 적용</li>
                                <li>• <strong>PDF 다운로드</strong>: 각 템플릿별 결과 비교</li>
                                <li>• <strong>통계 확인</strong>: 템플릿별 공간 효율성 비교</li>
                            </ul>
                        </div>
                    </div>
                </div>
