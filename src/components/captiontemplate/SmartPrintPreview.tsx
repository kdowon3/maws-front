...
const SmartPrintPreview: React.FC<SmartPrintPreviewProps> = ({
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && !stylesInjected) {
            ...
            return () => {
                const existingStyle = document.getElementById('smart-print-preview-styles');
                if (existingStyle) {
    ...
    const calculateLayout = async () => {
        ...
        try {
            console.log('스마트 레이아웃 계산 시작:', { artworkCount: artworks.length });

            // 1단계: 캡션 크기 측정 (템플릿 이름 전달)
            const measurements = await measureCaptions(artworks, TemplateComponent, undefined, templateName);
    ...
    const handleSmartPDFDownload = async () => {
        if (!pdfRef.current || layouts.length === 0) {
            console.log('PDF 다운로드 조건 실패:', { pdfRef: !!pdfRef.current, layoutsLength: layouts.length });
            return;
        }

        console.log('PDF 다운로드 시작:', { layoutsLength: layouts.length, artworksLength: artworks.length });
        ...
        try {
            ...
            for (let i = 0; i < layouts.length; i++) {
                const pageElement = pdfRef.current.querySelector(`[data-page="${i}"]`) as HTMLElement;
                ...
                console.log(`페이지 ${i} 요소 크기:`, {
                    ...
                    captionContents: Array.from(pageElement.querySelectorAll('.caption-item')).map(item => item.innerHTML.substring(0, 100) + '...'),
                });
                ...
                console.log(`페이지 ${i} 실제 크기:`, { actualWidth, actualHeight, rect });
                ...
                const canvas = await html2canvas(pageElement, {
                    ...
                    onclone: (clonedDoc) => {
                        ...
                        const clonedElement = clonedDoc.querySelector(`[data-page="${i}"]`) as HTMLElement;
                        if (clonedElement) {
                            console.log('클론된 페이지 요소 발견:', clonedElement);
                            
                            // 클론된 문서에 우리 스타일을 다시 주입
                            const existingStyle = clonedDoc.getElementById('smart-print-preview-styles');
                            if (!existingStyle) {
                            ...
                            const captionItems = clonedElement.querySelectorAll('.caption-item');
                            console.log(`클론된 문서에서 발견된 캡션 아이템 수: ${captionItems.length}`);
                            ...
                            captionItems.forEach((item, idx) => {
                                ...
                                console.log(`캡션 아이템 ${idx} 내용:`, htmlItem.innerHTML.substring(0, 100) + '...');
                            });
                        }
                    },
                });
    ...
    const renderCaption = (caption: CaptionMeasurement, index: number) => {
        ...
        return (
            <div
                ...
                data-artwork-title={caption.artwork?.title_ko || caption.artwork?.title_en}
            >
    ...
    const renderPage = (page: PageLayout, pageIndex: number) => (
        ...
        >
            ...
            >
                {page.rightColumn.map((caption, index) => renderCaption(caption, index))}
            </div>
        </div>
    );
        ...
        <div className="smart-print-preview bg-gray-100 p-6 rounded-lg">
            ...
            <div className="mb-4 flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-gray-600">2단 컬럼으로 공간을 효율적으로 활용하여 용지를 절약합니다</p>
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        disabled={artworks.length === 0 || layouts.length === 0 || isCalculating}
            ...
            <div ref={pdfRef} className="smart-layout-container">
                {artworks.length === 0 ? (
                    ...
                    <div className="text-center text-gray-500 py-20 bg-white rounded border">
                        ...
                        <p>레이아웃을 계산하고 있습니다...</p>
                        <p className="text-sm">캡션 개수가 많으면 시간이 걸릴 수 있습니다.</p>
                    </div>
                ) : layouts.length === 0 ? (
                    <div className="text-center text-gray-500 py-20 bg-white rounded border">
                    ...
                    <div className="pages-container">{layouts.map((page, index) => renderPage(page, index))}</div>
                )}
            </div>
            ...
            {layouts.length > 0 && (
                <div className="mt-4 text-sm text-gray-600">
                    ...
                    <p className="text-xs">* 각 캡션이 페이지 중간에서 짤리지 않도록 지능적으로 배치됩니다.</p>
                    <p className="text-xs">* 2단 컬럼으로 공간을 효율적으로 활용합니다.</p>
                    <p className="text-xs">* PDF 다운로드 버튼을 클릭하여 최적화된 레이아웃으로 저장할 수 있습니다.</p>
                </div>
            )}
