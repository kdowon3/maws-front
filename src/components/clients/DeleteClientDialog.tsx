...
export default function DeleteClientDialog({
  ...
  const handleDelete = async () => {
    ...
    if (
      ...
      (client.id.includes("_") || client.id.includes("row"))
    ) {
      console.log("임시 ID 또는 old row ID이므로 서버 삭제 불가:", client.id);
      alert(
        "이 고객은 아직 서버에 저장되지 않았거나 잘못된 형식입니다. 페이지를 새로고침 후 다시 시도해주세요.",
      );
      return;
    }
    ...
    try {
      ...
      alert(
        "고객 삭제에 실패했습니다. 페이지를 새로고침 후 다시 시도해주세요.",
      );
    }
  };
