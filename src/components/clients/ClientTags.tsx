...
const ClientTags: React.FC<ClientTagsProps> = ({
    ...
    return (
        <div className={`relative ${className}`}>
            {/* 선택된 태그 표시 */}
            <div className={`flex flex-wrap items-center ${readonly ? 'gap-1 justify-center' : 'gap-2 mb-2'}`}>
                <div className="relative" data-tag-trigger ref={dropdownRef}>
                    <Badge
                        ...
                        className={`flex items-center gap-1 transition-all duration-200 border ${
                            readonly
                                ? 'py-0.5 px-2 text-xs mx-auto max-w-[120px] justify-center' // 테이블용 컴팩트 스타일 + 가운데 정렬 + 너비 제한
