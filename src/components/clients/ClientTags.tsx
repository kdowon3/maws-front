...
const ClientTags: React.FC<ClientTagsProps> = ({
  ...
  // 기본 태그 상태 관리
  const [defaultTag, setDefaultTag] = useState<TagType>({
    id: -1,
    name: "일반고객",
    color: "#6B7280",
  });
  ...
  const [selectedTag, setSelectedTag] = useState<TagType>(
    initialTags.length > 0 ? initialTags[0] : defaultTag,
  );
  ...
  useEffect(() => {
    ...
    const fetchTags = async () => {
      try {
        ...
        // 기본 태그 '일반고객'이 없으면 생성
        const defaultTagExists = tags.some(
          (tag: TagType) => tag.name === "일반고객",
        );
        if (!defaultTagExists) {
          try {
          ...
          // 현재 선택된 태그가 임시 기본 태그면 실제 태그로 교체
          if (selectedTag.id === -1) {
            const realDefaultTag = tags.find(
              (tag: TagType) => tag.name === "일반고객",
            );
            if (realDefaultTag) {
  ...
  return (
    <div className={`relative ${className}`}>
      {/* 선택된 태그 표시 */}
      <div
        className={`flex flex-wrap items-center ${readonly ? "gap-1" : "gap-2 mb-2"}`}
      >
        <div className="relative" data-tag-trigger ref={dropdownRef}>
          ...
          >
            <span
              ...
              className={readonly ? "font-medium" : ""}
            >
              {selectedTag.name}
            </span>
