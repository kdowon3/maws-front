...
interface ClientsCardViewProps {
    clients: any[]; // Client 타입을 제거하고 any[]로 변경
    handleClientAction: (actionType: string, clientId: number) => void;
    artworks: any[];
}

const ClientsCardView: React.FC<ClientsCardViewProps> = ({ clients, handleClientAction, artworks }) => {
    // 고객별 구매작가명 추출 함수
    const getPurchasedArtists = (clientId: number) => {
        return Array.from(
            new Set(
                (artworks || [])
                            ...
                            <CardContent className="p-0">
                                <div className="p-4">
                                    ...
                                    <div className="flex flex-col gap-2 mt-4">
                                        ...
                                        <div className="flex items-center justify-between text-sm">
                                            <span className="text-gray-500">구매작가명</span>
                                            <span>{getPurchasedArtists(client.id).join(', ') || '없음'}</span>
                                        </div>
                                        <div className="flex items-center justify-between text-sm">
                                            <span className="text-gray-500">관심작가명</span>
                                            <span>
                                                {Array.isArray(client.favoriteArtist)
                                                    ? client.favoriteArtist.join(', ')
                                                    : client.favoriteArtist || '없음'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-3">
                                        ...
                                        <div className="flex flex-wrap gap-1">{client.note || '-'}</div>
                                    </div>
                                </div>
