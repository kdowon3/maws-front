...
const CustomerFilter: React.FC<CustomerFilterProps> = ({
    ...
    const handleApplyFilters = () => {
        ...
        if (selectedFilters.includes('vip')) {
            filteredClients = filteredClients.filter(
                (client) => client.data?.vip === true || client.data?.VIP === true || client.data?.등급 === 'VIP'
            );
        }

        if (selectedFilters.includes('regular')) {
            filteredClients = filteredClients.filter(
                (client) => client.data?.단골 === true || client.data?.regular === true || client.data?.등급 === '단골'
            );
        }

        if (selectedFilters.includes('purchaseHistory')) {
            filteredClients = filteredClients.filter(
                (client) => client.data?.구매이력 === true || client.data?.purchase_history === true
            );
        }
        ...
        if (selectedArtists.length > 0) {
            filteredClients = filteredClients.filter((client) => {
                return selectedArtists.some(
                    (artist) => client.data?.artist === artist || client.data?.관심작가 === artist
                );
            });
        }
    ...
    return (
        <Card className="p-6">
                ...
                <TabsContent value="filters" className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        {loading ? (
                            ...
                            <p className="text-sm text-gray-500">등록된 작가 정보가 없습니다.</p>
                        )}
                    </div>
                    ...
                    <div className="max-h-64 overflow-y-auto space-y-2">
                        {loading ? (
                            ...
                            searchedClients.map((client) => {
                                ...
                                const canReceiveMessage = hasPhone && client.data?.문자수신동의 !== false;
                                
                                return (
                                    <div
                                        key={client.id}
                                        className={`flex items-center space-x-3 p-3 border rounded-md hover:bg-gray-50 cursor-pointer ${
                                            isSelected ? 'bg-brand-blue/5 border-brand-blue' : 'border-gray-200'
            ...
            <Button className="w-full mt-4 bg-brand-blue hover:bg-brand-blue/90" onClick={handleApplyFilters}>
                필터 적용하기
            </Button>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                            문자 수신 동의:{' '}
                            {
                                filteredClients.filter((client) => client.data?.문자수신동의 !== false && client.phone)
                                    .length
                            }
                            명
                        </span>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                            문자 수신 거부:{' '}
                            {filteredClients.filter((client) => client.data?.문자수신동의 === false).length}명
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-yellow-500"></span>
                        <span className="text-sm">
                            연락처 없음: {filteredClients.filter((client) => !client.phone).length}명
                        </span>
                    </div>
                </div>
            </div>
