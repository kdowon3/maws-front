...
export async function issueArtworkCertificate(id) {
    const res = await authenticatedFetch(`${API_BASE_URL}/artworks/${id}/issue_certificate/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    });
...
export async function fetchClientDetail(id: number) {
    const res = await authenticatedFetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/clients/${id}/`);
    if (!res.ok) throw new Error('Failed to fetch client detail');
    return res.json();
}

export async function fetchArtworkDetail(id: number) {
    const res = await authenticatedFetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/artworks/${id}/`);
...
export async function addClient(data: any) {
    ...
    const payload = {
        name: name || '',
        phone: phone || '',
        tag_ids: tags ? tags.map((tag: any) => (typeof tag === 'object' ? tag.id : tag)) : [],
        data: rest,
    };
    ...
    if (!res.ok) throw new Error('고객 추가 실패');
    return res.json();
}
...
export async function updateClient(id: number, data: any) {
    ...
    // 태그 ID 추출 (임시 ID -1 제외)
    // tags가 없으면 기존 태그를 유지 (빈 배열로 설정하지 않음)
    const tagIds =
        tags !== undefined
            ? tags
                  .filter((tag: any) => tag.id !== -1) // 임시 기본 태그 제외
                  .map((tag: any) => (typeof tag === 'object' ? tag.id : tag))
            : undefined; // undefined로 설정하여 기존 태그 유지

    // 기존 데이터와 새로운 데이터를 병합 (기존 데이터 보존, 태그 관련 정보 제외)
    const cleanRest = { ...rest };
    // 태그 관련 키들 제거
    const tagRelatedKeys = ['tags', 'tag', '고객분류', 'customer_tags', 'tag_ids'];
    ...
    const payload: any = {
        ...
        data: { ...existingClient.data, ...cleanRest }, // 기존 data와 새로운 data 병합 (태그 정보 제외)
    };

    // tag_ids가 undefined가 아닐 때만 payload에 추가
    if (tagIds !== undefined) {
        payload.tag_ids = tagIds;
    }

    console.log('🔧 updateClient 요청:', { id, originalData: data, existingClient, payload });
...
export async function createClientColumn(column: { header: string; accessor: string; type: string; order: number }) {
    const res = await authenticatedFetch(`${API_BASE_URL}/client-columns/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export async function deleteClientColumn(id: number) {
    const res = await authenticatedFetch(`${API_BASE_URL}/client-columns/${id}/`, {
        method: 'DELETE',
    });
...
export async function updateClientColumn(
    ...
    console.log('🔧 updateClientColumn 호출:', { id, data });
    console.log('🔧 API URL:', `${API_BASE_URL}/client-columns/${id}/`);
    ...
    console.log('🔧 updateClientColumn 응답:', { status: res.status, ok: res.ok });

    if (!res.ok) {
        const errorText = await res.text();
        console.error('🔧 updateClientColumn 실패:', res.status, errorText);
        throw new Error(`컬럼 수정 실패 (${res.status}): ${errorText}`);
    }

    const result = await res.json();
    console.log('🔧 updateClientColumn 성공:', result);
    return result;
}
...
export async function getAllTags() {
    ...
    // 페이지네이션 형식(results 배열) 또는 직접 배열인지 확인
    if (data && typeof data === 'object' && 'results' in data && Array.isArray(data.results)) {
        return data.results;
    } else if (Array.isArray(data)) {
        console.log('🏷️ 직접 배열 형식 감지:', data);
        return data;
    } else {
        console.error('🚨 예상치 못한 태그 응답 형식:', data);
        return [];
    }
}
...
export async function createTag(name: string, color?: string) {
    ...
    if (!res.ok) {
        const errorText = await res.text();
        console.error('❌ createTag 실패:', res.status, errorText);

        // 중복 태그 에러 처리
        if (res.status === 400 && errorText.includes('이미 존재합니다')) {
            console.log('🏷️ 중복 태그 감지 - 기존 태그 검색 시도');

            try {
                // 모든 태그를 가져와서 같은 이름의 태그 찾기
                const allTags = await getAllTags();
                const existingTag = allTags.find((tag: any) => tag.name === name.trim());

                if (existingTag) {
                    console.log('🏷️ 기존 태그 찾음:', existingTag);
                    return existingTag;
                }
            } catch (findError) {
                console.error('기존 태그 검색 실패:', findError);
            }
        }

        throw new Error(`태그 생성 실패: ${errorText}`);
    }
    return res.json();
}
...
export async function filterClientsByTag(tagIds: number[]) {
    ...
    const res = await authenticatedFetch(`${API_BASE_URL}/clients/filter-by-tag/?${params.toString()}`);
    if (!res.ok) throw new Error('태그 필터링 실패');
    return res.json();
}

// 하위 호환성을 위한 래퍼 함수들 (기존 코드에서 사용 중인 경우)
export async function addClientTag(clientId: number, tag: string | { id: number; name: string }) {
...
export async function removeClientTag(clientId: number, tag: string | { id: number; name: string }) {
    const client = await fetchClientDetail(clientId);
    const currentTags = client.tags || [];
...
export async function analyzeExcelHeaders(headers: string[]) {
    const res = await authenticatedFetch(`${API_BASE_URL}/excel/analyze-headers/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export async function updateClientTagsOnly(clientId: number, tags: any[]) {
    console.log('🏷️ updateClientTagsOnly 호출:', { clientId, tags });

    // tags가 이미 ID 배열인지 객체 배열인지 확인
    let tagIds;
    if (tags.length > 0 && typeof tags[0] === 'number') {
        // 이미 ID 배열인 경우
        tagIds = tags.filter((id: number) => id !== -1);
        console.log('🏷️ 이미 ID 배열로 받음:', tagIds);
    } else {
        // 객체 배열인 경우 ID 추출
        tagIds = tags
            .filter((tag: any) => tag.id !== -1) // 임시 기본 태그 제외
            .map((tag: any) => (typeof tag === 'object' ? tag.id : tag));
        console.log('🏷️ 객체에서 ID 추출:', tagIds);
    }

    console.log('🏷️ 최종 tagIds:', tagIds);

    const payload = { tag_ids: tagIds };
    console.log('🏷️ 서버로 보낼 payload:', payload);

    const res = await authenticatedFetch(`${API_BASE_URL}/clients/${clientId}/tags/`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
    });
    ...
    if (!res.ok) {
        const errorText = await res.text();
        console.error('🏷️ 태그 업데이트 실패:', res.status, errorText);
        throw new Error(`태그 업데이트 실패 (${res.status}): ${errorText}`);
    }

    const result = await res.json();
    console.log('🏷️ updateClientTagsOnly 성공:', result);
    return result;
}
