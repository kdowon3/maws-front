...
export const measureCaptions = async (
  ...
  return new Promise((resolve) => {
    ...
    artworks.forEach((artwork, index) => {
      ...
      const captionElement = createCaptionElementByTemplate(
        artwork,
        index,
...
// 템플릿별 캡션 HTML 요소 생성
const createCaptionElementByTemplate = (
  artwork: any,
  index: number,
  templateName: string,
): HTMLElement => {
  // 공통 크기 정보 조합
  const width =
    artwork.width && artwork.width !== "" ? Number(artwork.width) : null;
  const height =
    artwork.height && artwork.height !== "" ? Number(artwork.height) : null;
  const depth =
    artwork.depth && artwork.depth !== "" ? Number(artwork.depth) : null;
...
const createTemplate6Element = (
  ...
  captionDiv.className =
    "border border-dashed border-gray-300 p-3 min-w-[140px] max-w-[180px] text-center bg-white";
  captionDiv.style.cssText = `
        min-width: 140px;
        max-width: 180px;
        padding: 12px;
...
const createTemplate7Element = (artwork: any, details: string): HTMLElement => {
  const captionDiv = document.createElement("div");
  captionDiv.className =
    "bg-white border border-dashed border-gray-300 p-2 min-w-[100px] max-w-[180px]";
  captionDiv.style.cssText = `
        min-width: 100px;
        max-width: 180px;
        padding: 8px;
...
const createTemplate8Element = (
  ...
  const captionDiv = document.createElement("div");
  captionDiv.className =
    "bg-white border border-dashed border-gray-300 p-2 min-w-[120px] max-w-[220px]";
  captionDiv.style.cssText = `
        min-width: 120px;
        max-width: 220px;
        padding: 8px;
...
export const calculateOptimalLayout = (
  ...
  for (let i = 0; i < captions.length; i++) {
    ...
    const canFitInLeftColumn =
      currentPage.leftHeight + caption.height <= config.maxPageHeight;
    const canFitInRightColumn =
      currentPage.rightHeight + caption.height <= config.maxPageHeight;
    ...
    if (canFitInLeftColumn) {
      ...
      console.log(
        `→ 왼쪽 열에 배치 (높이: ${currentPage.leftHeight}mm/${config.maxPageHeight}mm, 캡션: ${caption.height}mm)`,
      );
    }
    // 2순위: 오른쪽 열에 완전히 들어가는지 확인
      ...
      console.log(
        `→ 오른쪽 열에 배치 (높이: ${currentPage.rightHeight}mm/${config.maxPageHeight}mm, 캡션: ${caption.height}mm)`,
      );
    }
    // 3순위: 새 페이지 생성 (캡션이 페이지 경계에 걸치지 않도록)
    else {
      // 현재 페이지 마무리
      currentPage.totalHeight = Math.max(
        currentPage.leftHeight,
        currentPage.rightHeight,
      );
      ...
      if (caption.height > config.maxPageHeight) {
        console.error(
          `⚠️ 경고: 캡션 ${caption.id}의 높이(${caption.height}mm)가 페이지 최대 높이(${config.maxPageHeight}mm)를 초과합니다!`,
        );
      ...
      currentPage.leftHeight += caption.height;
      console.log(
        `→ 새 페이지 ${currentPage.pageNumber} 왼쪽 열에 완전 배치 (${caption.height}mm)`,
      );
    }
  }
...
export const optimizeColumnBalance = (pages: PageLayout[]): PageLayout[] => {
  return pages.map((page) => {
    ...
    if (
      heightDiff > 20 &&
      page.leftColumn.length > 1 &&
      page.rightColumn.length > 1
    ) {
      console.log(
        `페이지 ${page.pageNumber} 밸런싱 시작 (차이: ${heightDiff}mm)`,
      );
...
const balancePageColumns = (page: PageLayout): PageLayout => {
  ...
  console.log(
    `밸런싱 대상 캡션 ${allCaptions.length}개:`,
  ...
  balancedPage.totalHeight = Math.max(
    balancedPage.leftHeight,
    balancedPage.rightHeight,
  );
  ...
  console.log(
    `밸런싱 결과: ${originalDiff}mm → ${newDiff}mm (개선: ${originalDiff - newDiff}mm)`,
  );
  console.log(
    `최종 높이: 왼쪽 ${balancedPage.leftHeight}mm, 오른쪽 ${balancedPage.rightHeight}mm`,
  );
...
const balancePageColumnsBestFit = (page: PageLayout): PageLayout => {
  ...
  if (allCaptions.length <= 8) {
    ...
    console.log(
      `Best Fit 결과: ${Math.abs(page.leftHeight - page.rightHeight)}mm → ${bestDiff}mm`,
    );
  } else {
    // 캡션이 많으면 휴리스틱 방법 사용
...
const balancePageColumnsHeuristic = (page: PageLayout): PageLayout => {
  ...
  for (const caption of sortedCaptions) {
    const leftDiff = Math.abs(
      balancedPage.leftHeight + caption.height - targetHeight,
    );
    const rightDiff = Math.abs(
      balancedPage.rightHeight + caption.height - targetHeight,
    );
...
export const getLayoutStats = (pages: PageLayout[]) => {
  const totalCaptions = pages.reduce(
    (sum, page) => sum + page.leftColumn.length + page.rightColumn.length,
    0,
  );

  const averageHeight =
    pages.reduce((sum, page) => sum + page.totalHeight, 0) / pages.length;

  const efficiency =
    pages.reduce((sum, page) => {
      const utilization =
        page.totalHeight / DEFAULT_LAYOUT_CONFIG.maxPageHeight;
  ...
  return {
    ...
    estimatedPaperSaving: Math.max(
      0,
      Math.round(((totalCaptions - pages.length) / totalCaptions) * 100),
    ),
  };
};
